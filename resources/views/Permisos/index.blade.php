...
--}}
<div 
    x-data="permissionsTable({
        users: {{ Js::from($permisos->map(fn($p) => [
            'id' => $p->id,
            'sitio' => $p->sitio,
            'ruta' => $p->ruta,
            'status' => $p->status,
...
>
    ...
    <div class="bg-white p-6 rounded-2xl shadow mb-6 flex flex-wrap items-center gap-6">
        ...
        <div class="flex-1">
            <label class="text-sm font-semibold text-gray-700">Buscar permiso</label>
            <input 
                x-model="search"
                @input="page = 1"
                type="text"
                                ...
                                <a :href="permiso.url_gestionar" 
                                   class="inline-flex items-center justify-center 
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v12a2 2 0 002 2h12a2 2 0 002-2v-5m-5-5l5 5m0 0V7m0 5h-5"/>
                                    </svg>
                                    Gestionar
                                </a>
                            </template>
    ...
    >
        <div class="text-sm text-gray-500">
            Mostrando <span class="font-bold text-gray-900" x-text="startItem"></span> - <span class="font-bold text-gray-900" x-text="endItem"></span> de <span class="font-bold text-gray-900" x-text="filteredData.length"></span>
        </div>
        
        <div class="flex gap-2">
            <button @click="prevPage" :disabled="page === 1" class="px-3 py-1 rounded-lg border bg-white disabled:opacity-50 hover:bg-gray-100 transition text-sm">Anterior</button>
            <button @click="nextPage" :disabled="page === totalPages" class="px-3 py-1 rounded-lg border bg-white disabled:opacity-50 hover:bg-gray-100 transition text-sm">Siguiente</button>
        </div>
    </div>
        ...
        Alpine.data('permissionsTable', (config) => ({
            ...
            get filteredData() {
                return this.items.filter(item => {
                    const matchesSearch = item.sitio.toLowerCase().includes(this.search.toLowerCase()) || 
            ...
            get startItem() { return this.filteredData.length === 0 ? 0 : (this.page - 1) * this.pageSize + 1; },
            get endItem() { return Math.min(this.page * this.pageSize, this.filteredData.length); },
