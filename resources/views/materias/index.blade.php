    ...
    <div x-data="{
        ...
        <div class="mb-6 flex flex-col md:flex-row items-start md:items-center justify-between">
            <div class="flex items-center gap-3">
                <div
                    class="w-10 h-10 rounded-xl bg-gradient-to-br from-[#2FA69A] to-[#23877E] flex items-center justify-center shadow-lg">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-[#E2E8F0] mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                ...
                <div class="md:col-span-2 relative">
                    ...
                    <input type="text" x-model.debounce.300ms="searchTerm" placeholder="Buscar por Nombre o Código..."
                ...
                <div class="flex gap-2">
                    <select x-model="filterStatus"
                        class="border border-[#E2E8F0] rounded-xl bg-white py-2 px-3 focus:ring-2 focus:ring-[#2FA69A] focus:border-[#2FA69A] w-full">
                        <option value="Todas">Estado</option>
                        <option value="Activa">Activa</option>
                        <option value="Inactiva">Inactiva</option>
                    </select>

                    <select x-model="filterHours"
                        class="border border-[#E2E8F0] rounded-xl bg-white py-2 px-3 focus:ring-2 focus:ring-[#2FA69A] focus:border-[#2FA69A] w-full">
            ...
            <div class="border-t border-slate-200 pt-4">
                ...
                <div class="flex space-x-3 pb-2 overflow-x-auto">
                    {{-- Tab 'Todas' --}}
                    <button @click="activeCarrera = 'Todas'"
                        :class="activeCarrera === 'Todas' ? 'bg-[#2FA69A] text-white shadow-md' :
                            'bg-slate-100 text-slate-600 hover:bg-[#2FA69A]/10'"
                    ...
                    <template x-for="carrera in carreras" :key="carrera.id">
                        <button @click="activeCarrera = carrera.id"
                            :class="activeCarrera === carrera.id ? 'bg-[#2FA69A] text-white shadow-md' :
                                'bg-slate-100 text-slate-600 hover:bg-[#2FA69A]/10'"
                            class="flex-shrink-0 px-5 py-3 rounded-xl font-semibold transition duration-150 text-sm whitespace-nowrap">
                            <span x-text="carrera.nombre_carrera || carrera.nombre"></span>
                            <span class="text-xs opacity-75 ml-1"
                                x-text="'(' + materiasData.filter(m => m.carreras && m.carreras.some(c => c.id == carrera.id)).length + ')'"></span>
                        </button>
                    </template>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            <template x-for="(materia, index) in paginatedMaterias" :key="materia.id">
                <div x-transition:enter="transition ease-out duration-300"
                    ...
                    :style="`transition-delay: ${index * 60}ms;`" {{-- Animación de entrada escalonada --}} {{-- ESTILO DE CARD Y HOVER --}}
                    ...
                    <div class="bg-gradient-to-r from-[#2FA69A] to-[#23877E] p-4 rounded-t-2xl text-white relative">
                        <div class="flex items-center justify-between">
                            ...
                            <span x-text="materia.status == 1 ? 'Activa' : 'Inactiva'"
                    ...
                    <div class="p-5 bg-white rounded-b-2xl">
                        <div class="flex items-start space-x-3 mb-4">
                            ...
                            <div
                                class="w-12 h-12 bg-[#2FA69A]/15 rounded-xl text-[#2FA69A] flex items-center justify-center shadow-inner mt-1">
                            ...
                            <h4 x-text="materia.nombre" class="text-xl font-extrabold text-gray-800 leading-snug"
                                :title="materia.nombre"></h4>
                        </div>

                        {{-- Descripción truncada a 3 líneas --}}
                        <p x-text="materia.descripcion" class="text-sm text-slate-500 leading-relaxed line-clamp-3 mb-4">
                        </p>
                        ...
                        <div class="space-y-2 text-sm">
                            <p class="flex items-center gap-2 text-gray-700 font-medium">
                                ...
                                <span
                                    x-text="materia.carreras.length ? materia.carreras.map(c => c.nombre_carrera || c.nombre).join(', ') : 'Sin carrera asignada'"
                                    class="truncate"></span>
                            </p>
                            <p class="flex items-center gap-2 text-gray-700">
                                ...
                                <span class="font-bold text-[#2FA69A]" x-text="materia.horas_semanales + 'h '"></span>
                                <span class="text-gray-500">/ Sem.</span> |
                                <span class="font-bold text-[#2FA69A]" x-text="materia.horas_totales + 'h '"></span> <span
                                    class="text-gray-500">/ Total</span>
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-[#F8FAFC] border-t border-[#E2E8F0] p-4 rounded-b-2xl">
                        <div class="flex justify-between items-center gap-2">
                            ...
                            @if (Auth::user()->hasPermission('materias.asignarMateria'))
                                <button @click="selectedMateria = materia; openAsignarModal = true"
                            ...
                            <button @click="editMateria(materia)"
                                class="p-2 text-gray-500 hover:text-white hover:bg-[#2FA69A] rounded-lg transition duration-200"
                                title="Editar">
            ...
            <div x-show="filteredMaterias.length === 0"
                class="col-span-full text-center py-12 text-gray-500 transition duration-300">
                No se encontraron materias registradas con los filtros actuales.
            </div>
        </div>

        {{-- PAGINACIÓN --}}
        <div x-show="filteredMaterias.length > itemsPerPage"
            class="mt-8 flex justify-between items-center bg-white p-4 rounded-xl shadow-md">
            <span class="text-sm text-gray-700">
                Mostrando
                <span class="font-semibold"
                    x-text="Math.min((currentPage - 1) * itemsPerPage + 1, filteredMaterias.length)"></span>
                a
            ...
            <div class="inline-flex space-x-2">
                <button @click="currentPage = Math.max(1, currentPage - 1)" :disabled="currentPage === 1"
                    :class="currentPage === 1 ? 'bg-gray-200 text-gray-500 cursor-not-allowed' :
                        'bg-[#2FA69A] text-white hover:bg-[#23877E]'"
                ...
                <button @click="currentPage = Math.min(totalPages, currentPage + 1)"
                    ...
                    :class="currentPage === totalPages ? 'bg-gray-200 text-gray-500 cursor-not-allowed' :
                        'bg-[#2FA69A] text-white hover:bg-[#23877E]'"
                    class="px-3 py-1 rounded-lg font-semibold transition duration-150">
                    Siguiente
                </button>
        ...
        {{-- MODAL CREAR MATERIA --}}
        {{-- ****************************************************** --}}
        <div x-show="openCreateModal" style="display: none;" x-transition.opacity
            class="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-50 p-4">
            <div x-transition.scale @click.outside="openCreateModal = false"
                ...
                <form action="{{ route('materias.store') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="nombre" required
                            class="mt-1 block w-full rounded-lg border border-gray-300 shadow-sm focus:ring-2 focus:ring-[#2FA69A] focus:border-[#2FA69A] p-3">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="carrera_id" required
                            class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-[#2FA69A] focus:border-[#2FA69A] p-3">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="horas_semanales" required
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-[#2FA69A] focus:border-[#2FA69A] p-3">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Total Horas</label>
                            <input type="number" name="horas_totales" required
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-[#2FA69A] focus:border-[#2FA69A] p-3">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="descripcion" name="descripcion" rows="3"
                            class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-[#2FA69A] focus:border-[#2FA69A] p-3"></textarea>
                    </div>

                    <div class="flex justify-end gap-3 pt-4">
                        <button type="button" @click="openCreateModal = false"
                            class="px-5 py-2 rounded-lg bg-gray-200 hover:bg-gray-300 transition text-[#0F172A] shadow-sm">Cancelar</button>
                        <button type="submit"
                            class="px-6 py-2 rounded-lg bg-[#2FA69A] text-white font-bold hover:bg-[#23877E] transition shadow-lg">Guardar
                            Materia</button>
                    </div>
                </form>
            </div>
        ...
        {{-- ****************************************************** --}}
        <div x-show="openAsignarModal" style="display: none;" x-transition.opacity
            class="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-50 p-4">
            <div x-transition.scale @click.outside="openAsignarModal = false"
                class="bg-white rounded-xl shadow-lg p-8 w-full max-w-lg relative max-h-[90vh] overflow-y-auto border border-gray-200">
                ...
                <form action="{{ route('materias.asignarMateria') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="docente_id" {{-- Nombre cambiado de docentes[] a docente_id --}} required
                    ...
                    <div class="flex justify-end gap-3 pt-4">
                        <button type="button" @click="openAsignarModal = false"
                            class="px-5 py-2 rounded-lg bg-gray-200 hover:bg-gray-300 transition text-[#0F172A] shadow-sm">Cancelar</button>
                        <button type="submit"
                            class="px-6 py-2 rounded-lg bg-[#2FA69A] text-white font-bold hover:bg-[#23877E] transition shadow-lg">Guardar
                            Asignación</button>
        ...
        {{-- ****************************************************** --}}
        <div x-show="openEditModal" style="display: none;" x-transition.opacity
            class="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-50 p-4">
            <div x-transition.scale @click.outside="openEditModal = false"
                class="bg-white rounded-xl shadow-lg p-8 w-full max-w-lg relative max-h-[90vh] overflow-y-auto border border-gray-200">
                <h2 class="text-2xl font-bold text-[#2FA69A] mb-6 border-b pb-3">Editar Materia</h2>

                <form :action="selectedMateria ? '/materias/' + selectedMateria.id : '#'" method="POST"
                    class="space-y-4">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="text" name="nombre" :value="selectedMateria?.nombre" required
                            class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm p-3 focus:ring-[#2FA69A]">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700">Código</label>
                        <input type="text" name="codigo" :value="selectedMateria?.codigo" required
                            class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm p-3 focus:ring-[#2FA69A]">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="carrera_id"
                            class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm p-3 focus:ring-[#2FA69A]">
                            <template x-for="c in carreras" :key="c.id">
                                <option :value="c.id" x-text="c.nombre_carrera || c.nombre"
                                    :selected="selectedMateria?.carreras.some(sc => sc.id == c.id)"></option>
                            </template>
                        </select>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Carga Semanal</label>
                            <input type="number" name="horas_semanales" :value="selectedMateria?.horas_semanales"
                                required
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm p-3 focus:ring-[#2FA69A]">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Total Horas</label>
                            <input type="number" name="horas_totales" :value="selectedMateria?.horas_totales" required
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm p-3 focus:ring-[#2FA69A]">
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end gap-3 pt-4">
                        <button type="button" @click="openEditModal = false"
                            class="px-5 py-2 rounded-lg bg-gray-200 hover:bg-gray-300 transition text-[#0F172A] shadow-sm">Cancelar</button>
                        <button type="submit"
                            class="px-6 py-2 rounded-lg bg-[#2FA69A] text-white font-bold hover:bg-[#23877E] transition shadow-lg">Actualizar</button>
                    </div>
                </form>
            </div>
        </div>
